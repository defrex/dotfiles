...
function gwt() {
    ...
    if [ -n "$existing_worktree" ]; then
        echo "Worktree already exists for branch '$1' at: $existing_worktree"
    ...
    if git rev-parse --verify "$1" >/dev/null 2>&1; then
        echo "Branch '$1' already exists, checking it out in new worktree"
        git worktree add "$worktree_dir" "$1"
    else
        echo "Creating new branch '$1' from $default_branch"
        git worktree add -b "$1" "$worktree_dir" "$default_branch"
    fi && \
    if [ -f .env ]; then
